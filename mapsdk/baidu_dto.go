...
func (d *baiduReverseGeocodingResponse3) ResponseDTO(uri string) *Response[ReverseGeocodingData] {
	...
	for _, item := range d.Result.POIs {
		child = append(child, Location{
			ID:           item.UID,
			Name:         item.Name,
			Address:      str.Pick(item.Address, d.Result.FormattedAddress),
			Country:      "",
			Province:     str.Pick(item.Province, ac.Province),
			City:         str.Pick(item.City, ac.City),
			Area:         str.Pick(item.Area, ac.District),
			Street:       "",
			Town:         item.Town,
			StreetNumber: "",
			Tags:         []string{},
