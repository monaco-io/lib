...
const (
	WGS84 TYPE = "wgs84" // GPS原始坐标
	GCJ02 TYPE = "gcj02" // 国测局加密坐标（火星坐标）
	BD09  TYPE = "bd09"  // 百度加密坐标
)

func (v TYPE) IsOK() bool {
	return v == WGS84 || v == GCJ02 || v == BD09
}

// Transform 坐标转换主函数
func Transform(lon, lat float64, from, to TYPE) (longitude float64, latitude float64) {
	if from == to {
		return lon, lat
	}
	switch from {
...
func transform(lon, lat float64) (x, y float64) {
	lonlat := lon * lat
	absX := math.Sqrt(math.Abs(lon))
	lonPi, latPi := lon*math.Pi, lat*math.Pi
	d := 20.0*math.Sin(6.0*lonPi) + 20.0*math.Sin(2.0*lonPi)
	x, y = d, d
